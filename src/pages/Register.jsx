...
export const Register = () => {
    const [username, setUsername] = useState("michelle@primiani.com")
    const [password, setPassword] = useState("michelle")
    const [firstName, setFirstName] = useState("Michelle")
    const [lastName, setLastName] = useState("Primiani")
        ...
        fetch(`http://localhost:8000/register`, {
            ...
            .then(authInfo => {
                if (authInfo && authInfo.token) {
                    localStorage.setItem("book_token", JSON.stringify(authInfo))
                    navigate("/")
                } else {
